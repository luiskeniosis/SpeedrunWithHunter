...
Concept:
Players are split into teams of hunters and runners in a 1or2:1 ratio.
This means that if there are 2 players, 1 is a hunter, and 1 is a runner.
If there are 3 players, 2 are hunters, 1 is a runner
...
Runners only have one life. However, they have constant speed 1, and wooden and stone tools they craft come with efficiency 1.

Runners kits are known as professions, which helps them gather resources (Lumberjack, Miner, etc), or help them escape (Swimmer, Digger, etc).

Hunters kits are known as monsters, that help them catch runners (Trapper, etc), or to kill them faster (Werewolf, etc).
...
There is no grace period when the game starts. All players are randomly spread in a 75 block RADIUS, meaning they can spawn on the same block, or up to 150 blocks away. All up to luck. Runners are given a burst of speed and haste when they first walk on land. Hunters are given their player trackers.

If a single runner enters the end, hunters in the nether and the overworld will gain a huge speed boost until they enter the end.
...
Once all players are ready, teams are randomly assigned (can be changed manually by an admin), and players are given kit selectors. Those on the runners team will get to choose from runner kits, those on the hunters team will get to choose from hunter kits (duh).
...
If the hunter is in the overworld: if there are no runners in the overworld, but there is at least 1 runner in the nether, the compass will change its tracking to the nearest nether portal. If there are no runners in the overworld nor the nether, the compass will change its tracking to the nearest end portal.

If the hunter is in the nether: if there are no runners in the nether, the compass will change its tracking to the nether portal that was last used in the nether, as well as display the portal’s coordinates.

If the hunter is in the end: if there are no runners in the end, the hunter is either too early, or too late. The compass will stop tracking in the end either way. This means that in the End, hunters can not track runners, regardless of where any runner is.

Empowered Abilities:
Each hunter becomes empowered through gameplay. Every hunter will be empowered in 30 minutes, but can be sped up by doing kit specific actions or by attacking/killing runners.

Custom Runner Interactions/Recipes:
Runners have access to custom recipes to help defend against various hunters.
-Crafting arrows with Allium will create Garlic coated arrows, which will poison and slow Vampires.
...
-Apples with bone meal to duplicate apples
-Golden apples with just 4 ingots rather than 9
...
Redstone Block
Redstone
Can only be applied to pickaxes, automatically smelts ores mined. Overrides silk touch, includes fortune.
Furnace
Book
Furnace
Coal Block
...
In the Nether..
Players will have the coordinates of the portal they came through on a sidebar. They have resistance 1 if set on fire until they’re no longer on fire. Runners are given 7 absorption hearts when they first enter the nether

In the End..
Runners will have constant regeneration and strength, fall damage will be reduced by 60%. Runners will gain 2 respawns per hunter +1, hunters will gain 1 respawn per runner +1 (Ex. in a 1v1, runner will have 3 respawns, hunter will have 2). There will be no endermen, only hunters, runners, and the dragon. Dragon will take 40% less damage. Runners are given 10 absorption hearts when they first enter the end
...
Kit Miner
Start with a wooden pickaxe, when mining ores (with any pickaxe), every ore connected to the one you broke also breaks. Every 3 stone, stone variant, or ore broken will grant a burst of haste. (low cooldown)
...
WEAKNESS:
-Exiting the spectral realm grants you weakness, slowness and mining fatigue for 25 seconds. It will also briefly have you appear glowing.
